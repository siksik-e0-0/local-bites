...
function sanitizePatch(raw: unknown): PlaceEditPayload | { error: string } {
  ...
  if ("businessHours" in obj) {
    if (obj.businessHours === null) {
      out.businessHours = null;
    } else if (typeof obj.businessHours === "string") {
      const h = obj.businessHours.trim();
      out.businessHours = h ? h.slice(0, 500) : null;
    } else {
